(function ($)
{
    ...
    window._lg_subwin_width = 800;
    window._lg_subwin_height = 400;
    ...
    LG.ajax = function (options)
    {
        ...
        var url = p.url || ashxUrl + $.param({ type: p.type, method: p.method }).replace("type=","").replace("method=","").replace("&","/");
        $.ajax({
            cache: false,
            async: true,
            url: url,
    ...
    {
        var menuno = $("#MenuNo").val() || $("#MenuNo").attr("value");
        if (!menuno && getQueryStringByName)
        {
    ...
    LG.bulidFilterGroup = function (form)
    {
        ...
        .each(function ()
        {
            ...
            var ltype = $(this).attr("ltype");//通用查询需要设置以下属性
            ...
            //通用查询需要设置以下属性
            //attr: { op: "equal" ,ltype:"select"}, cssClass: "field",
            //add by lhy 2014-03-14
            if (ltype == "select")
            {
                value = $.ligerui.get(name).getValue();
                name = name;
            }
            group.rules.push({
                op: op,
    ...
    LG.appendSearchBtn = function (form, onClick)
    {
        if (!form) return;
        form = $(form);
        //搜索按钮 附加到第一个li
        var container = $('<ul><li style="margin-right:8px;clear:left;"></li></ul><div class="l-clear"></div>').appendTo(form);
    ...
    LG.setFormReadOnly = function (mainform)
    {
        if (!mainform)
            mainform = $("form:first"); 
    ...
    LG.loadButtons = function (options)
    {
        var p = $.extend({
            menuno: LG.getPageMenuNo(),
            loading: '正在加载按钮中...',
            callback: function () { }
        }, options || {});
        LG.ajax({
            loading: '正在加载工具条中...',
            type: 'button',
            method: 'getUserButtons',
            data: { MenuNo: p.menuno },
            success: function (data)
            {
            	p.callback(data);
            }
    ...
    //明细页 - 关闭自身
    LG.closeCurrentDetailPage = function (reloadParent)
    {
        if (parent && parent.homepage)
        {
            parent.LG.closeCurrentTab(null);
        }
        else if (parent)
        {
            if (reloadParent && parent.f_reload)
            {
                parent.f_reload();
            }
            parent.LG.closeDetailPage();
        }
        else
        {
            window.close();
    ...
    LG.showChildPage =
    LG.showDetailPage = function (tabid, text, path, pagename, subShow)
    {
        if (top.f_addTab && !subShow)
        {
            top.f_addTab(tabid, text, path + "/" + pagename);
        }
        else
        {
            LG.openSubWindow({ url: pagename, title: text });
        }
    };

    LG.openSubWindow = function (p)
    {
        var options = $.extend(true, {
            width: _lg_subwin_width, height: _lg_subwin_height, isResize: true, isDrag: true, title: '明细页'
        }, p);
        window.detailWin = $.ligerDialog.open(options);
    };
    ...
    LG.FormToJSON = function (formValues) {
		var result = {};
		for ( var formValue, j = 0; j < formValues.length; j++) {
			formValue = formValues[j];
			var name = formValue.name;
			var value = LG.encodeSpecialCode(formValue.value.trim());
			if (name.indexOf('.') < 0) {
				result[name] = value;
				continue;
			} else {
	 ...
	 * 对特殊字符和中文编码
	 */
	LG.encodeSpecialCode = function (res)
	{
